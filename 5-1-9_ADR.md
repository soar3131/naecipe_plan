...
## ADR-010: 이벤트 브로커 단계 도입 (Kafka 조건부)

### 메타데이터
- **상태**: Accepted
- **결정일**: 2026.02.19
- **결정자**: Tech Lead, Backend, Infra

### 컨텍스트
문서 전반에서 Kafka를 기본 전제로 두고 있으나, Phase 1 MVP 범위와 운영 복잡도/비용 대비 효익을 재검토할 필요가 있음.

### 결정
- **Phase 1(MVP): Outbox + 단일 큐 기반 이벤트 처리 채택**
- **Phase 2+: Kafka/MSK는 도입 게이트 충족 시 전환**

### 도입 게이트
1. 이벤트 처리량이 단일 큐 한계에 근접
2. 다중 소비자 리플레이/장기 보존 요구 명확
3. 운영팀이 Kafka 운영 역량/모니터링 체계 확보

### 결과
- MVP 단순화 및 출시 속도 개선
- Kafka 도입 시점을 명확한 조건 기반으로 관리
- 이벤트 스키마 호환성 유지가 전환의 선행 조건
